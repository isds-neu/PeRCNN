...
def train(model, init_state, n_iters, learning_rate, dt, dx, save_path):
    # model
    ...
    for epoch in range(n_iters):
        ...
        print('[%d/%d %d%%] Epoch loss: %.15f, ' % ((epoch+1), n_iters, ((epoch+1)/n_iters*100.0), loss.item()))
...
if __name__ == '__main__':
    ...
    total_step = 400
    time_batch_size = 400  # not used since there is only one batch
    steps = total_step + 1
    ...
    start = time.time()
    train(model, init_state, n_iters, learning_rate, dt, dx, save_path)
    end = time.time()
