...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, output_channels, input_kernel_size,
        ...
        self.dx = 0.2
        self.dt = 0.0125
        self.nu_up = 0.2     # nu from 0 to upper bound (two times the estimate)
        ...
        self.W_laplace = nn.Conv2d(1, 1, self.input_kernel_size, self.input_stride, padding=self.input_padding,
        ...
        self.Wh1_u = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=self.input_padding, padding_mode='circular', bias=True, )
        self.Wh2_u = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=self.input_padding, padding_mode='circular', bias=True, )
        self.Wh3_u = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=self.input_padding, padding_mode='circular', bias=True, )
        self.Wh4_u = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=1,
                               stride=1, padding=0, bias=True)
        # Nonlinear term for v (up to 3rd order)
        self.Wh1_v = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=self.input_padding, padding_mode='circular', bias=True, )
        self.Wh2_v = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=self.input_padding, padding_mode='circular', bias=True, )
        self.Wh3_v = nn.Conv2d(in_channels=2, out_channels=16, kernel_size=self.input_kernel_size,
                               stride=self.input_stride, padding=self.input_padding, padding_mode='circular', bias=True, )
    ...
    def forward(self, h):
        ...
        u_res = self.nu_up*torch.sigmoid(self.CA)*self.W_laplace(u_prev) + self.Wh4_u( self.Wh1_u(h)*self.Wh2_u(h)*self.Wh3_u(h) )
        v_res = self.nu_up*torch.sigmoid(self.CB)*self.W_laplace(v_prev) + self.Wh4_v( self.Wh1_v(h)*self.Wh2_v(h)*self.Wh3_v(h) )
...
def pretrain_upscaler(Upconv, init_state_low, epoch=4000, plotFlag=True):
    ...
    if plotFlag:
        ...
        #
        cf = ax[0, 0].scatter(x_star, y_star, c=u_bicubic-u_upconv, alpha=0.9, edgecolors='none', cmap='RdBu', marker='s', s=4, vmin=-1, vmax=1)
        ax[0, 0].axis('square')
        ax[0, 0].set_xlim([0, 1])
        ax[0, 0].set_ylim([0, 1])
        ax[0, 0].set_xticks([])
        ...
        #
        cf = ax[0, 1].scatter(x_star, y_star, c=u_upconv, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
        ax[0, 1].axis('square')
        ax[0, 1].set_xlim([0, 1])
        ax[0, 1].set_ylim([0, 1])
        ax[0, 1].set_xticks([])
        ...
        #
        cf = ax[0, 2].scatter(x_star, y_star, c=u_low_res, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
        ax[0, 2].axis('square')
        ax[0, 2].set_xlim([0, 1])
        ax[0, 2].set_ylim([0, 1])
        ax[0, 2].set_xticks([])
        ...
        #
        cf = ax[1, 0].scatter(x_star, y_star, c=v_bicubic-v_upconv, alpha=0.9, edgecolors='none', cmap='RdBu', marker='s', s=4, vmin=-1, vmax=1)
        ax[1, 0].axis('square')
        ax[1, 0].set_xlim([0, 1])
        ax[1, 0].set_ylim([0, 1])
        ax[1, 0].set_xticks([])
        ...
        # #
        cf = ax[1, 1].scatter(x_star, y_star, c=v_upconv, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
        ax[1, 1].axis('square')
        ax[1, 1].set_xlim([0, 1])
        ax[1, 1].set_ylim([0, 1])
        ax[1, 1].set_xticks([])
        ...
        #
        cf = ax[1, 2].scatter(x_star, y_star, c=v_low_res, alpha=0.9, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
        ax[1, 2].axis('square')
        ax[1, 2].set_xlim([0, 1])
        ax[1, 2].set_ylim([0, 1])
        ax[1, 2].set_xticks([])
...
def postProcess(output, truth, low_res, xmin, xmax, ymin, ymax, num, fig_save_path):
    ...
    #
    cf = ax[0, 0].scatter(x_star, y_star, c=u_pred-u_star, alpha=1.0, edgecolors='none', cmap='RdBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[0, 0].axis('square')
    ...
    fig.colorbar(cf, ax=ax[0, 0], fraction=0.046, pad=0.04)
    #
    cf = ax[0, 1].scatter(x_star, y_star, c=u_star, alpha=1.0, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[0, 1].axis('square')
    ...
    fig.colorbar(cf, ax=ax[0, 1], fraction=0.046, pad=0.04)
    #
    cf = ax[0, 2].scatter(x_star, y_star, c=u_low_res, alpha=1.0, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[0, 2].axis('square')
    ...
    fig.colorbar(cf, ax=ax[0, 2], fraction=0.046, pad=0.04)
    #
    cf = ax[1, 0].scatter(x_star, y_star, c=v_pred-v_star, alpha=1.0, edgecolors='none', cmap='RdBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[1, 0].axis('square')
    ...
    fig.colorbar(cf, ax=ax[1, 0], fraction=0.046, pad=0.04)
    #
    cf = ax[1, 1].scatter(x_star, y_star, c=v_star, alpha=1.0, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[1, 1].axis('square')
    ...
    fig.colorbar(cf, ax=ax[1, 1], fraction=0.046, pad=0.04)
    #
    cf = ax[1, 2].scatter(x_star, y_star, c=v_low_res, alpha=1.0, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[1, 2].axis('square')
...
def postProcess_v2(output, truth, low_res, xmin, xmax, ymin, ymax, num, fig_save_path):
    ...
    fig.subplots_adjust(hspace=0.25, wspace=0.25)
    #
    cf = ax[0, 0].scatter(x_star, y_star, c=u_pred, alpha=1.0, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[0, 0].axis('square')
    ax[0, 0].set_xlim([xmin, xmax])
    ax[0, 0].set_ylim([ymin, ymax])
    ax[0, 0].set_xticks([])
    ...
    fig.colorbar(cf, ax=ax[0, 2], fraction=0.046, pad=0.04)
    #
    cf = ax[1, 0].scatter(x_star, y_star, c=v_pred, alpha=1.0, edgecolors='none', cmap='RdYlBu', marker='s', s=4, vmin=-1, vmax=1)
    ax[1, 0].axis('square')
    ax[1, 0].set_xlim([xmin, xmax])
    ax[1, 0].set_ylim([ymin, ymax])
    ax[1, 0].set_xticks([])
