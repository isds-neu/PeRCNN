...
if __name__ == '__main__':
    ...
    # Reconstructed data
    UV = sio.loadmat('../data/uv_2x201x100x100_[PeRCNN,41x51x51,5%noise,3layers].mat')['uv']
    ...
    u_term = {'ones*lap_u': 0.1, 'u*ones': 1.0, 'u**3*ones': -1, 'u*v**2*ones': -1,
    ...
    w_best = trainer.train(maxit = 20, STR_iters = 20, lam = 0.0001, d_tol = 5, kappa = 1)   # kappa = 0.5 for 30% noise
    ...
    # 0% noise  - {'ones*lap_u': 0.09465181027123706, 'u*ones': 1.00811925058762,   'u**3*ones': -1.0167560133141067, 'u**2*v*ones': 0.9973918248944547, 'u*v**2*ones': -1.0176977019111648, 'v**3*ones': 0.9981686664544911}
    # 5% noise  - {'ones*lap_u': 0.09678292864556487, 'u*ones': 0.9924811160896033, 'u**3*ones': -0.9970061502305889, 'u**2*v*ones': 1.0007173119011712, 'u*v**2*ones': -1.0004349618043336, 'v**3*ones': 0.9971526368027861}
    # 10% noise - {'ones*lap_u': 0.09915857907067026, 'u*ones': 0.9406264791183649, 'u**3*ones': -0.9348620127967582, 'u**2*v*ones': 1.0027055113584409, 'u*v**2*ones': -0.9416941117593453, 'v**3*ones': 0.997736623278785}
    # 20% noise - {'ones*lap_u': 0.1096812859274619, 'u*ones': 0.9058847966703432, 'u**3*ones': -0.8840997892078614, 'u**2*v*ones': 0.9997930433946041, 'u*v**2*ones': -0.896977703953944, 'v**3*ones': 0.9975257473604903}
    # 30% noise - {'ones*lap_u': 0.11337319581658156, 'u*ones': 0.8223828906348857, 'u**3*ones': -0.7849419629764204, 'u**2*v*ones': 1.0020766708111233, 'u*v**2*ones': -0.8052164877317389, 'v**3*ones': 1.003140144353781}
    ...
    with open('pareto_analysis.npy', 'wb') as f:
        np.save(f, [list_gamma, list_LS_loss, list_L0_loss])
    with open('./pareto_analysis.npy', 'rb') as f:
    ...
    plt.savefig('./select_gamma_u.png', dpi=300)
    plt.show()
    ...
    plt.savefig('./pareto_u.png', dpi=300)
    plt.show()
