...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, output_channels, input_kernel_size,
        ...
        self.nu_u = torch.nn.Parameter(torch.tensor(0.0050078, dtype=torch.float64), requires_grad=True)   # lap_u
        self.nu_v = torch.nn.Parameter(torch.tensor(0.0050228, dtype=torch.float64), requires_grad=True)   # lap_v

        self.C1_u = torch.nn.Parameter(torch.tensor(-0.982252, dtype=torch.float64), requires_grad=True)  # uu_x
        self.C2_u = torch.nn.Parameter(torch.tensor(-0.992132, dtype=torch.float64), requires_grad=True)  # vu_y

        self.C1_v = torch.nn.Parameter(torch.tensor(-0.983758, dtype=torch.float64), requires_grad=True)  # uv_x
        self.C2_v = torch.nn.Parameter(torch.tensor(-0.971269, dtype=torch.float64), requires_grad=True)  # vv_y
...
if __name__ == '__main__':
    ...
    # 5%
    # u_t=-0.005011\Delta u + 1.00426uu_x + 1.004305vu_y
    # v_t=-0.004995\Delta u + 1.00878uv_x + 1.001912vv_y
